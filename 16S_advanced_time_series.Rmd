...
metadata_plot <- as.data.frame(sample_data(ps2))
...
# Get sample metadata
metadata_all <- as.data.frame(sample_data(ps2))
...
baseline_dist <- data.frame(
  ...
  mean_dist_to_baseline = sapply(later_samples, function(s) {
    ...
    baseline_same_trt <- sample_names(subset_samples(ps_d0, treatment == trt))
...
gam_model <- gam(Observed ~ treatment + s(day_numeric, by = treatment),
                 data = alpha_df)
plot(gam_model)
