...
**First time setup:** If you haven't already, run the `setup_packages.R` script to install all required packages. You only need to do this once per Posit Cloud project.

### Check and Load Packages

This code chunk checks if packages are installed and provides helpful error messages if they're missing:

```{r load-libraries}
# Function to load packages and provide helpful error messages
load_package <- function(pkg) {
  if (!require(pkg, character.only = TRUE, quietly = TRUE)) {
    stop(paste0(
      "Package '", pkg, "' is not installed.\n",
      "Please run the setup_packages.R script first, or install with:\n",
      if (pkg %in% c("phyloseq", "DESeq2")) {
...
load_package("phyloseq")

# Data manipulation and visualization
load_package("tidyverse")

# Statistical testing
load_package("vegan")        # For PERMANOVA and ecological statistics
load_package("ggpubr")       # For adding statistics to plots

# Differential abundance testing
load_package("DESeq2")       # For identifying differentially abundant taxa

# Additional plotting
load_package("gridExtra")    # For arranging multiple plots
...
- **phyloseq:** Specialized for microbiome data - organizes sequences, taxonomy, and metadata together
...
- **DESeq2:** Statistical testing for count data, originally designed for RNA-seq
