...
**Optional: Save this figure**

If you want to save publication-quality figures for your reports or presentations, you can run the code below.

**How to use:** Change `eval=FALSE` to `eval=TRUE` in the chunk header above (line with `save-alpha-figure`), then run the chunk.

```{r save-alpha-figure, eval=FALSE}
# Create figures directory if it doesn't exist
dir.create("figures", showWarnings = FALSE)

# Save high-resolution figure for publications/reports
alpha_combined <- gridExtra::arrangeGrob(p_richness, p_shannon, ncol = 2)
ggsave("figures/alpha_diversity_over_time.png",
       plot = alpha_combined,
       width = 12, height = 5, dpi = 300)

cat("✓ Figure saved to: figures/alpha_diversity_over_time.png\n")
```
...
**Advanced tip: Customizing colors and shapes**

If you want more control over plot aesthetics (especially when plotting many categories), you can manually specify colors and shapes:

```r
# Example: Manual color palette
my_colors <- c("Vehicle" = "#E69F00", "Amp" = "#56B4E9",
               "Metro" = "#009E73", "AmpMetro" = "#D55E00")

# Example: Manual shapes (useful when you have >6 categories)
my_shapes <- c("D0" = 16, "D3" = 17, "D7" = 15, "D13" = 18)

# Apply to plot
plot_ordination(ps2, ord, color = "treatment", shape = "treatment_days") +
  geom_point(size = 3) +
...
This gives you precise control over visual appearance, which is especially useful for:
- Publication-quality figures
- Colorblind-friendly palettes
- Plots with many categories (>6 shapes or colors)
...
To save this comparison as a high-resolution image, change `eval=FALSE` to `eval=TRUE` in the chunk header.

```{r save-beta-figure, eval=FALSE}
# Create figures directory if it doesn't exist
dir.create("figures", showWarnings = FALSE)

# Save comparison of distance metrics
beta_combined <- gridExtra::arrangeGrob(
  p_beta_treatment + labs(title = "UniFrac (Phylogenetic)"),
  p_beta_bray + labs(title = "Bray-Curtis (Abundance-based)"),
  ncol = 2
)
ggsave("figures/beta_diversity_comparison.png",
       plot = beta_combined,
       width = 12, height = 5, dpi = 300)

cat("✓ Figure saved to: figures/beta_diversity_comparison.png\n")
```
...
To save this volcano plot for your reports, change `eval=FALSE` to `eval=TRUE` in the chunk header.

```{r save-deseq2-figure, eval=FALSE}
# Create figures directory if it doesn't exist
dir.create("figures", showWarnings = FALSE)

# Save volcano plot
ggsave("figures/deseq2_volcano_plot.png",
       plot = p_volcano,
       width = 8, height = 6, dpi = 300)

cat("✓ Figure saved to: figures/deseq2_volcano_plot.png\n")
```
...
# Statistical test - stratified by treatment
...
**Why test within each treatment group separately?**

Notice we tested the diversity-survival relationship *within* each treatment group, not pooled across all groups. This is critical because:

1. **Treatment affects both diversity AND survival** - Different antibiotics cause different diversity levels and different mortality rates
2. **Simpson's Paradox** - Pooling can mask or reverse relationships that exist within groups
3. **Biological interpretation** - The question is: "Does baseline diversity predict survival *given a treatment*?", not "Does diversity differ between treatments?"

**Example:** AmpMetro mice have low diversity AND high mortality. But *within* AmpMetro-treated mice, does even lower diversity predict worse survival? That's what stratified analysis reveals.

**Try it yourself (optional):** Test diversity vs survival pooled across all treatments. You'll likely get a borderline or non-significant result because the treatment effects dilute the within-treatment signal!
