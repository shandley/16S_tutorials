...
2. **Assess** data quality and make informed decisions about sample/taxon filtering
3. **Visualize** microbial community composition across experimental groups
4. **Calculate and interpret** alpha diversity metrics (richness and evenness)
5. **Perform and interpret** beta diversity analysis using ordination and PERMANOVA
6. **Identify** differentially abundant taxa between treatment groups
7. **Connect** microbiome patterns to biological outcomes (survival)
...
Before we begin, we need to install and load several R packages. This section teaches you about package management - an important skill for reproducible research.

**What are R packages?**

Packages are collections of functions, data, and documentation that extend R's capabilities. For microbiome analysis, we need specialized packages that aren't included with base R.

**Two types of packages:**

1. **CRAN packages** - General R packages installed with `install.packages()`
2. **Bioconductor packages** - Specialized biology/genomics packages installed with `BiocManager::install()`

### Step 1: Install Required Packages

**IMPORTANT:** Run this code chunk ONCE when you first open this tutorial. You do NOT need to run it again in future sessions.

```{r install-packages, eval=FALSE}
# This chunk has eval=FALSE so it won't run automatically when you knit
# You need to run it manually the first time
...
cat("=================================================\n")
# These are general R packages from CRAN
cran_packages <- c("tidyverse", "vegan", "ggpubr", "gridExtra")
...
cat("=================================================\n")
# These are specialized genomics packages from Bioconductor
...
cat("You can now run the next code chunk to load packages.\n")
cat("=================================================\n")
```

**What just happened?**

- We installed **BiocManager** (a tool for installing biology packages)
- We installed **4 CRAN packages** for data analysis and visualization
- We installed **2 Bioconductor packages** for microbiome analysis
- Each package only installs if it's not already present
- This takes 5-10 minutes total

**Installation time:**
- BiocManager: ~30 seconds
- CRAN packages: ~3-5 minutes
- Bioconductor packages: ~3-5 minutes
- **Total: 5-10 minutes**

### Step 2: Load Packages

**Run this chunk every time** you open the tutorial in a new R session:

```{r load-libraries, message=TRUE}
# This chunk loads packages that are already installed
# If a package is missing, you'll get a clear error message

cat("Loading required packages...\n\n")

# Core microbiome analysis (Bioconductor)
library(phyloseq)
cat("✓ phyloseq loaded\n")
...
# Differential abundance testing (Bioconductor)
library(DESeq2)
cat("✓ DESeq2 loaded\n")

# Additional plotting (CRAN)
library(gridExtra)
cat("✓ gridExtra loaded\n")

cat("\nAll packages loaded successfully!\n")
cat("You're ready to start the analysis.\n\n")
...
**If you get "there is no package called 'X'" error:**

1. Make sure you ran the **install-packages** chunk above
2. Wait for installation to complete (don't interrupt it)
3. Try running the **load-libraries** chunk again

**If installation fails:**

Try installing packages one at a time in the Console:
```r
# For CRAN packages:
...
- **tidyverse:** Modern R tools for data manipulation (dplyr) and visualization (ggplot2)
- **vegan:** Ecological statistics including PERMANOVA for comparing communities
- **DESeq2:** Statistical testing for count data (finds differentially abundant bacteria)
- **ggpubr:** Adds statistical annotations to plots
- **gridExtra:** Arranges multiple plots into panels
...
sample_summary <- data.frame(
  ...
  left_join(as.data.frame(sample_data(ps1)), by = "sample_id")
...
4. Antibiotic-treated samples (especially Amp+Metro) have **fewer reads**, reflecting depleted microbiomes
5. This is **real biology**, not technical failure!

**Biological insight:** Antibiotics are working! They're depleting the gut bacteria.
...
alpha_df <- alpha_div %>%
  ...
  left_join(as.data.frame(sample_data(ps2)), by = "sample_id")
...
sampledf <- as.data.frame(sample_data(ps2))
unifrac_dist <- phyloseq::distance(ps2, method = "unifrac")

# Test: Does treatment affect community composition?
set.seed(12345)
permanova_treatment <- adonis2(unifrac_dist ~ treatment,
                               data = sampledf,
                               permutations = 999)
cat("PERMANOVA: Effect of Treatment\n")
print(permanova_treatment)

# Test: What about timepoint?
permanova_time <- adonis2(unifrac_dist ~ treatment_days,
                         data = sampledf,
                         permutations = 999)
cat("\nPERMANOVA: Effect of Timepoint\n")
print(permanova_time)

# Combined model: Treatment + Time
permanova_both <- adonis2(unifrac_dist ~ treatment + treatment_days,
                         data = sampledf,
                         permutations = 999)
cat("\nPERMANOVA: Combined Model\n")
print(permanova_both)

# Pairwise comparisons between treatments
# (Using Day 7 as example)
ps2_d7 <- subset_samples(ps2, treatment_days == "D7")
sampledf_d7 <- as.data.frame(sample_data(ps2_d7))
unifrac_dist_d7 <- phyloseq::distance(ps2_d7, method = "unifrac")

set.seed(12345)
permanova_d7 <- adonis2(unifrac_dist_d7 ~ treatment,
                        data = sampledf_d7,
                        permutations = 999)
cat("\nPERMANOVA at Day 7 Only:\n")
print(permanova_d7)
```

**Interpretation:**
- **R²** tells you how much variation is explained
- **p-value < 0.05** means groups differ significantly
